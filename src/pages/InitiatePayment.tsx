...
const InitiatePayment: React.FC = () => {
  ...
  const normalizeStatus = (status: string): TransactionStatus => {
    ...
    if (s === 'in_transit' || s === 'in-transit') return TransactionStatus.IN_TRANSIT;
    return s as TransactionStatus;
  };

  const normalizedStatus = normalizeStatus(transaction.status);
  const canPay = normalizedStatus === TransactionStatus.PENDING;
  const isPaid = [
      ...
      <div className="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-6 animate-fade-in">
        ...
        {isPaid ? (
          <div className="max-w-2xl mx-auto animate-fade-in-up">
            <div className="bg-[var(--bg-card)] rounded-2xl shadow-lg border border-[var(--border-default)] overflow-hidden">
              <div className="bg-green-50 dark:bg-green-900/20 p-8 text-center border-b border-[var(--border-default)]">
                <div className="w-20 h-20 mx-auto bg-green-100 dark:bg-green-800 rounded-full flex items-center justify-center mb-4 shadow-sm">
                  <CheckCircle className="h-10 w-10 text-green-600 dark:text-green-300" />
                </div>
                <h2 className="text-2xl font-bold text-[var(--text-primary)] mb-2">Payment Completed</h2>
              ...
              <div className="p-6 sm:p-8 space-y-6">
                <div className="space-y-4">
                  <div className="flex justify-between items-center py-3 border-b border-[var(--border-default)]">
                    ...
                    <span className="font-mono font-medium text-[var(--text-primary)]">#{transaction.transaction_id}</span>
                  </div>
                  <div className="flex justify-between items-center py-3 border-b border-[var(--border-default)]">
                    <span className="text-[var(--text-secondary)]">Amount Paid</span>
                    <span className="text-xl font-bold text-[var(--text-primary)]">₵{maskAmount(transaction.amount)}</span>
                  </div>
                  <div className="flex justify-between items-center py-3 border-b border-[var(--border-default)]">
                    <span className="text-[var(--text-secondary)]">Status</span>
                    <span className="px-3 py-1 rounded-full bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300 text-sm font-medium capitalize">
